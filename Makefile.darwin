     ...
     RMANBASENAME = rman
      RMANOPTIONS = RmanOptions
        ...
        DEPENDSRC = $(CONFIGSRC)/util
          INCROOT = /opt/homebrew/include
        USRLIBDIR = /opt/homebrew/lib
           VARDIR = /var
        VARLIBDIR = $(VARDIR)/lib
  SYSTEMUSRLIBDIR = /usr/lib
  SYSTEMUSRINCDIR = /usr/include
         SHLIBDIR = /opt/homebrew/lib
       LINTLIBDIR = $(USRLIBDIR)/lint
          MANPATH = /opt/homebrew/man
...
CXXDEPENDINCLUDES =
 CXXEXTRA_DEFINES =
CXXEXTRA_INCLUDES =
   CXXSTD_DEFINES = -D__aarch64__ -D__DARWIN__                                 -DNO_ALLOCA -DCSRG_BASED  $(CXXPROJECT_DEFINES)
       CXXOPTIONS =
...
MODLDCOMBINEFLAGS = -X -r
  ...
  STD_CPP_DEFINES = -traditional -D__GNUC__ -traditional -D__GNUC__ -D__aarch64__ -D__DARWIN__                                 -DNO_ALLOCA -DCSRG_BASED  $(PROJECT_DEFINES)
      STD_DEFINES = -D__aarch64__ -D__DARWIN__                                 -DNO_ALLOCA -DCSRG_BASED  $(PROJECT_DEFINES)
 EXTRA_LOAD_FLAGS =
  EXTRA_LDOPTIONS =
  EXTRA_LIBRARIES =
             TAGS = ctags
   ...
   BUILDMODULETOP = $(BUILDLIBTOP)/..
    XBUILDINCROOT = $(XTOP)/exports
     ...
     XBUILDBINDIR = $(XBUILDINCROOT)/bin
           INCDIR = $(INCROOT)
           ADMDIR = /usr/adm
           LIBDIR = /opt/homebrew/lib/X11
         SHAREDIR = /opt/homebrew/share/X11
       LIBEXECDIR = /opt/homebrew/libexec
        MODULEDIR = $(USRLIBDIR)/modules
   TOP_X_INCLUDES = -I/opt/homebrew/include
          XBINDIR = $(PROJECTROOT)/bin
       INSTSRCDIR = /opt/homebrew/src
        ETCX11DIR = /etc/X11
          CONFDIR = $(ETCX11DIR)
           DOCDIR = $(LIBDIR)/doc
     ...
     ENCODINGSDIR = ${datarootdir}/fonts/X11/encodings
...
RANDOM		= -DHAS_48
# Set your C compiler like the following example
# CC	= gcc
# ############################################################
...
install:: ${MYPROGRAM}
	...
	$(INSTALL) $(INSTALLFLAGS) $(INSTPGMFLAGS)  ${MYPROGRAM} $(DESTDIR)$(BINDIR)/${MYPROGRAM}

all:: ${MYPROGRAM}.$(MANNEWSUFFIX)

${MYPROGRAM}.$(MANNEWSUFFIX): ${MYPROGRAM}.$(MANSRCSUFFIX)
	$(RM) $@
	if test -z "$(PREPROCESSMANPAGES)" ; then \
	   cd `dirname ${MYPROGRAM}` && \
	   $(LN) `basename ${MYPROGRAM}.$(MANSRCSUFFIX)` `basename $@`; \
	else \
	      	     $(RAWCPP)  $(MANDEFS) $(EXTRAMANDEFS) \
	     < ${MYPROGRAM}.$(MANSRCSUFFIX) | sed -e '/^#  *[0-9][0-9]*  *.*$$/d' 			-e '/^#line  *[0-9][0-9]*  *.*$$/d' 			-e '/^[ 	]*XCOMM$$/s/XCOMM/#/' 			-e '/^[ 	]*XCOMM[^a-zA-Z0-9_]/s/XCOMM/#/' 			-e '/^[ 	]*XHASH/s/XHASH/#/' 			-e '/\@\@$$/s/\@\@$$/\\/' >$@; \
	fi

cleandir::
	$(RM) ${MYPROGRAM}.$(MANNEWSUFFIX)

install.man:: ${MYPROGRAM}.$(MANNEWSUFFIX)
	...
	$(INSTALL) $(INSTALLFLAGS) $(INSTMANFLAGS) ${MYPROGRAM}.$(MANNEWSUFFIX) $(DESTDIR)$(MANDIR)/${MYPROGRAM}.$(MANSUFFIX)
